...
class CodexSemanticExtractor:
    ...
    def extract(self, dna: dict) -> dict:
        ...
        prompt = (
            ...
            "a subject with evidence in the same viewport/state. imagery.subject_{x,y,width,height}_ratio, imagery.subject_visibility, imagery.subject_crop and imagery.subject_kind also require relative_to naming their enclosing media region. Other features must not have relative_to. "
            ...
            "name/scope/relative_to combinations again or reinterpret structured geometry, appearance or media estimates. Media-local ratios must never become viewport ratios; visible fragment bounds must never become full-element bounds. "
            ...
            "Explicit approximate viewport ratios in inferred evidence may be extracted as inferred ratios. "
            "Cite that estimate as supporting evidence, preserve its coordinate frame and uncertainty, and obey "
            "its confidence ceiling; do not cite only the qualitative observation beneath it. Never convert "
            "image-normalized estimates into CSS pixels or use cropped/full-page image bounds as viewport bounds. "
