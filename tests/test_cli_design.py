...
class CliDesignTests(unittest.TestCase):
    ...
    def test_preservation_intent_and_timeout_reach_adapter(self):
        with TemporaryDirectory() as directory:
            ...
            factory.assert_called_once_with(intent="preserve", timeout_seconds=450, estimate_geometry=False, geometry_regions=(), appearance_regions=(), media_regions=())

    def test_geometry_opt_in_reaches_adapter(self):
        with TemporaryDirectory() as directory:
            path = Path(directory) / "viewport.png"; path.write_bytes(b"image")
            analyzer = Mock()
            analyzer.analyze.side_effect = lambda references, targets: profile(references, targets)
            with patch("visparse.cli.CodexDesignAnalyzer", return_value=analyzer) as factory, redirect_stdout(io.StringIO()):
                self.assertEqual(main(["analyze-design", str(path), "--estimate-geometry",
                                       "--geometry-region", "hero", "--geometry-region", "headline",
                                       "--appearance-region", "headline", "--media-region", "hero"]), 0)
            factory.assert_called_once_with(intent="adapt", timeout_seconds=300, estimate_geometry=True,
                                           geometry_regions=("hero", "headline"), appearance_regions=("headline",), media_regions=("hero",))
